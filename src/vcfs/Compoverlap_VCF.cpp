...
int overlap_bothdir(strvcfentry vcf1, std::vector<strvcfentry> vcf2,
		int max_dist) {
	for (size_t i = 0; i < vcf2.size(); i++) {
		//check type:
		if (vcf2[i].type == vcf1.type) {
			//check chrs:
			if (strcmp(vcf2[i].stop.chr.c_str(), vcf1.stop.chr.c_str()) == 0
					&& strcmp(vcf2[i].start.chr.c_str(), vcf1.start.chr.c_str())
							== 0) {
				//check coordinates:
				if (abs(vcf2[i].stop.pos - vcf1.stop.pos) < max_dist
						&& abs(vcf2[i].start.pos - vcf1.start.pos) < max_dist) {
					return i;
				}
			}

			if (strcmp(vcf2[i].start.chr.c_str(), vcf1.stop.chr.c_str()) == 0
					&& strcmp(vcf2[i].stop.chr.c_str(), vcf1.start.chr.c_str())
							== 0) {
				//check coordinates:
				if (abs(vcf2[i].start.pos - vcf1.stop.pos) < max_dist
						&& abs(vcf2[i].stop.pos - vcf1.start.pos) < max_dist) {
					return i;
				}
			}
		}
...
void print_header(std::string vcf_file, FILE *& out) {
	...
	if (!myfile.good()) {
		std::cout << "BED Parser: could not open file: " << vcf_file.c_str()
				<< std::endl;
		exit(0);
	}
...
void print_entry(strvcfentry entry, FILE *& out) {
	...
	for (std::map<std::string, std::string>::iterator tz = entry.calls.begin();
			tz != entry.calls.end(); tz++) {
		fprintf(out, "%s", (*tz).second.c_str());
	}
	fprintf(out, "%c", '\n');
}

void comp_overlap_vcf(std::string vcf1_file, std::string vcf2_file,
		int max_dist, std::string output) {
...
//0=DEL,1=DUP,2=INV,3=TRA
	std::cout << "Overlap VCF1:" << " DEL " << overlap_vcf1[0] << " DUP "
			<< overlap_vcf1[1] << " INV " << overlap_vcf1[2] << " TRA "
			<< overlap_vcf1[3] << std::endl;
	std::cout << "Overlap VCF2:" << " DEL " << overlap_vcf2[0] << " DUP "
			<< overlap_vcf2[1] << " INV " << overlap_vcf2[2] << " TRA "
			<< overlap_vcf2[3] << std::endl;
	std::cout << "Uniqe VCF1:" << " DEL " << unique_vcf1[0] << " DUP "
			<< unique_vcf1[1] << " INV " << unique_vcf1[2] << " TRA "
			<< unique_vcf1[3] << std::endl;
	std::cout << "Uniqe VCF2:" << " DEL " << unique_vcf2[0] << " DUP "
			<< unique_vcf2[1] << " INV " << unique_vcf2[2] << " TRA "
			<< unique_vcf2[3] << std::endl;
